...
## Acceptance criteria
- [x] `open_append(path) -> std::ofstream` opens/creates the log file for appending
- [x] `append_block(std::ofstream&, const Block&)` writes one JSON object + newline and flushes
- [x] Parent directory is created if missing
- [x] Write errors surface as a thrown `StorageError` (typed exception), never `std::abort`/crash

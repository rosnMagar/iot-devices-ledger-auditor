...
## Acceptance criteria
- [ ] `load_chain(path) -> Chain` reads the log line-by-line into a `Chain`
- [ ] Empty/missing file → a new `Chain` seeded with genesis
- [ ] Malformed line throws a clear error (does not silently skip)
- [ ] Returns a `Chain` ready for use by the server's shared state

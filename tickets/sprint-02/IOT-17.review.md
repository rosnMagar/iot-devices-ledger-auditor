...
## Acceptance criteria
- [x] Test: append N blocks → `load_chain` → identical chain (roundtrip)
- [x] Test: corrupt a line on disk → reload → `verify()` fails at that index
- [x] Test: empty/missing file → `load_chain` seeds genesis
- [x] Tests use a unique temp dir per case (no shared global state); `ctest` green
